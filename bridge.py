...
def iscale(X: np.ndarray, bridge: np.ndarray, plot = False):
    inverse = RobustScaler().fit(X).inverse_transform(bridge)
...
def separate(X: np.ndarray, labels: np.ndarray, plot = False, threshold = 3e2):
...
def Z(X: np.ndarray, bridge: np.ndarray, edges: np.ndarray, prop = .05, idx = None, bNum = None, total = None, plot = False):
    '''
    X is the original (unclustered) dataset to correctly apply the inverse scaling 
    '''
    
    e1, e2 = edges
    x, y, z = xyz(iscale(X, bridge))
    # x, y, z = xyz(bridge)
    ...
    e1Height = np.mean(z[eP1[:n]])
    e2Height = np.mean(z[eP2[:n]])
    ...
    return lowest, e1Height, e2Height, *location
...
if __name__ == '__main__':
    ...
    for idx in range(19):
        ...
        for i,bridge in enumerate(bridges):
            ...
            out[count] = Z(d, bridge, edges, idx = idx, bNum = i, total = tot, plot = True)
            count += 1
            # h[i] = Z(d, bridge, edges, idx = idx, bNum = i, total = tot, plot = True)
            # print(h[i])
    df = pd.DataFrame(out)
    df.to_excel('Bridge/output.xlsx')